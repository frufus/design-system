...
## 3. Hand-write the rest of `src/tokens.css`

- [ ] 3.1 Add the non-colour tokens from the approved canvas: font stacks, the
...
- [ ] 3.2 Add the reduced-motion block collapsing all three durations in one place.
...
## 4. Prove both appearances are complete

- [ ] 4.1 Write the appearance-parity test: every token declared in light is
...
- [ ] 4.2 Write the contrast test over the shipped values, reusing the measured
      pair list.
...
- [ ] 5.1 Write the `@theme` nulling block setting Tailwind's default colour, font,
      radius and shadow scales to `initial`.
      **Verified by:** `npm run test -- registers-nulled` fails if a default scale
      is still reachable.
- [ ] 5.2 Write the `@theme inline` remap claiming the utility names, every one
...
- [ ] 5.3 Write the shared control register in `@layer components`: height, radius,
...
## 6. Enforcement and wiring

- [ ] 6.1 Add the literal-colour check over every package file except
      `src/tokens.css`.
      **Verified by:** `npm run lint` fails when a hex literal is added to a
      stylesheet or component.
- [ ] 6.2 Confirm the `exports` map resolves both stylesheets from a consuming
      path shape.
      **Verified by:** `npm run test -- exports` resolves `./tokens.css` and
      `./registers.css` to files that exist.
- [ ] 6.3 Record the token naming as an ADR via the `adr` skill.
      **Verified by:** the ADR file exists under `docs/adr/` and names what a
      rename would cost.

## 7. Close the change

- [ ] 7.1 Run the full suite plus `openspec validate --strict`.
      **Verified by:** both green, with the test count reported.
- [ ] 7.2 Update `CLAUDE.md` commands with `build:tokens`.
