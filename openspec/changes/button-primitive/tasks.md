...
- [ ] 1.1 Write `tests/class-maps.test.ts`: every documented variant and size has
      ...
      **Verified by:** `npm run test -- class-maps` fails when an entry is
      removed.
- [ ] 1.2 Implement `src/classMaps.ts` until 1.1 is green.
      **Verified by:** the same run, with no test skipped.
- [ ] 1.3 Extend the enforcement check to fail on a class name assembled from a
      variable.
...
- [ ] 2.1 Write `tests/button.test.ts` for behaviour only: it emits on click,
      ...
      **Verified by:** `npm run test -- button` — every case fails for want of a
      component.
- [ ] 2.2 Implement `src/components/Button.vue` until 2.1 is green.
      **Verified by:** the same run.
- [ ] 2.3 Add the accessible-name cases: a slotted label names the button, and an
...
- [ ] 3.1 Wire variant, size and state to the class maps, and take the focus ring
...
- [ ] 3.2 Give the small size a 44 px target without changing its visible box.
      **Verified by:** a test asserts the target-restoring class is present at
      the small size and absent at the others.

## 4. The catalog entry

- [ ] 4.1 Write `stories/Button.stories.ts` with the canvas's matrix: four
...
- [ ] 5.1 Export `Button` and its prop types from `src/index.ts`.
      **Verified by:** a test imports the public entry point and finds them.
- [ ] 5.2 Run the full suite, lint, typecheck, format and the catalog build.
      **Verified by:** all green, each on its own exit code, and `npm pack
      --dry-run` reports the new file count.
