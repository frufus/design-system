...
- [x] 1.1 Write `tools/smoke-consumer.mjs`: pack the package, install the tarball
      ...
      **Verified by:** `npm run test:consumer` completes and reports the built
      asset sizes.
- [x] 1.2 Assert the built output rather than the exit code: tokens present, the
      ...
      **Verified by:** the same command fails when the `@source` line is removed
      from the fixture.
- [x] 1.3 Prove it catches a broken published surface.
...
- [x] 2.1 Write the `<style scoped>` check with its tests.
      **Verified by:** `npm run test -- conventions` fails when a scoped style
      block is added to a component.
- [x] 2.2 Move all four checks behind `tools/check-conventions.mjs` and simplify
      the lint script.
      **Verified by:** `npm run lint` runs all four and reports each by name.

## 3. The written rules

- [x] 3.1 Write `docs/DESIGN-LANGUAGE.md`: each rule, why it exists, and which
...
- [x] 4.1 Update `CLAUDE.md` with the new commands.
      **Verified by:** every command listed there runs.
- [x] 4.2 Full gate plus the smoke test.
