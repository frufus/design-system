...
- [ ] 1.1 Add Storybook 9 for Vue 3 with Vite, plus `@storybook/addon-a11y`, as
...
- [ ] 1.2 Add `npm run dev` and `npm run build:storybook`, and confirm the
...
- [ ] 2.1 Write `.storybook/preview.css` with the documented four lines - the
...
- [ ] 2.2 Import the preview stylesheet from `.storybook/preview.ts`.
...
- [ ] 3.1 Add a global `appearance` type that writes `data-theme` onto the
...
- [ ] 3.2 Confirm no story receives an appearance as a prop.
...
- [ ] 4.1 Write a story rendering bare markup carrying `.fds-control`, with the
      a11y addon active.
...
- [ ] 5.1 Run the full suite, lint, typecheck, format and the catalog build.
      **Verified by:** all green, each on its own exit code.
- [ ] 5.2 Update `CLAUDE.md` commands.
