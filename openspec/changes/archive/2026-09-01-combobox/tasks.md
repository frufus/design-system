...
- [x] 1.1 Write `tests/combobox-filter.test.ts`: the default matcher is a
...
- [x] 1.2 Implement the matcher until 1.1 is green.
      **Verified by:** the same run.

## 2. The keyboard, case by case

- [x] 2.1 Write `tests/combobox.test.ts` covering every scenario in the spec:
      ...
      **Verified by:** `npm run test -- combobox` — every case fails for want of
      a component.
- [x] 2.2 Implement `src/components/Combobox.vue` until 2.1 is green.
      **Verified by:** the same run.

## 3. What it tells assistive technology

- [x] 3.1 Assert the ARIA surface: the roles, that the active-option reference
...
- [x] 3.2 Assert the live region: it reports the count in the project's words and
      stays empty when the project supplies none.
      **Verified by:** the same run.

## 4. Value integrity

- [x] 4.1 Assert that leaving with unmatched text restores rather than commits,
      and that a value outside the options shows empty text.
      **Verified by:** `npm run test -- combobox`.

## 5. Catalog and close

- [x] 5.1 Write stories: a long list, a custom matcher, an empty result, and one
      beside `Select` showing when each is the right answer.
      **Verified by:** the catalog builds and the stories are indexed.
- [x] 5.2 Export it, document it in the design language, and run the full gate
